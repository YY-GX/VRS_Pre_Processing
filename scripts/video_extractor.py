...
parser.add_argument("--vrs_file", type=str, default=None, help="Specify a single VRS file to process. If set, only this file will be processed.")
args = parser.parse_args()

# ---------------------------
# Configuration
# ---------------------------
VRS_ROOT = "./data/VRS/non_patients_vrs/"
...
DEVIGNETTING_MASK_FOLDER = "./projectaria_tools/devignetting_masks"
...
def process_image(provider, stream_id, frame_index, undistort, color_correct, devignette):
    ...
    # Only set color correction if not already color corrected
    if hasattr(provider, "is_color_corrected") and callable(getattr(provider, "is_color_corrected")):
        already_corrected = provider.is_color_corrected()
    elif hasattr(provider, "is_color_corrected"):
        already_corrected = provider.is_color_corrected
    else:
...
def find_smallest_vrs(root_dir):
    debug_file = "./data/VRS/useless_vrs/Try.vrs"
    return debug_file
...
# ---------------------------
if args.vrs_file is not None:
    vrs_path = os.path.join(VRS_ROOT, args.vrs_file)
    if not os.path.isfile(vrs_path):
        print(f"Specified VRS file does not exist: {vrs_path}")
    else:
        provider = data_provider.create_vrs_data_provider(vrs_path)
        if provider is None:
            print(f"Skipping invalid VRS: {vrs_path}")
        else:
            session_output_dir = os.path.join(OUTPUT_ROOT, os.path.splitext(os.path.basename(vrs_path))[0])
